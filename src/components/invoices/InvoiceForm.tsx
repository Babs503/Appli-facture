...
interface InvoiceFormProps {
  /** La facture à modifier (uniquement en mode édition) */
  invoice?: Invoice;
  /** true = modification d'une facture existante, false = création (défaut) */
  isEditing?: boolean;
}
...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ invoice, isEditing = false }) => {
  const navigate = useNavigate();
  const { clients, products, settings, addInvoice, updateInvoice } = useApp();
  ...
  const [date, setDate] = useState(invoice ? toInputDate(invoice.date) : toInputDate(new Date()));
  const [dueDate, setDueDate] = useState(invoice ? toInputDate(invoice.dueDate) : defaultDueDate());
  const [status, setStatus] = useState<Invoice['status']>(invoice?.status ?? 'draft');
  const [taxRate, setTaxRate] = useState<number>(invoice?.taxRate ?? settings.defaultTaxRate);
  const [shippingFee, setShippingFee] = useState<number>(invoice?.shippingFee ?? 0);
  const [paymentTerms, setPaymentTerms] = useState(invoice?.paymentTerms ?? settings.defaultPaymentTerms);
